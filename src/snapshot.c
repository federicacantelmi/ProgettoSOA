...
static void free_device_nm_rcu(struct rcu_head *rcu) {
    printk(KERN_INFO "%s: free_device_nm_rcu called\n", MODNAME);
    struct nonmounted_dev *p = container_of(rcu, struct nonmounted_dev, rcu_head);
    printk(KERN_INFO "%s: freeing non-mounted device\n", MODNAME);
    kfree(p);
    printk(KERN_INFO "%s: non-mounted device freed\n", MODNAME);
}
...
static void free_device_m_rcu(struct rcu_head *rcu) {
    printk(KERN_INFO "%s: free_device_m_rcu called\n", MODNAME);
    struct mounted_dev *p = container_of(rcu, struct mounted_dev, rcu_head);
    // kfree(p->dir_path);
    kfree(p->block_bitmap);
    kfree(p);
    printk(KERN_INFO "%s: mounted device freed\n", MODNAME);
}

static char *get_name(struct block_device *bdev) {

    printk(KERN_INFO "%s: get_name called for block device (major=%d, minor=%d)\n", MODNAME, MAJOR(bdev->bd_dev), MINOR(bdev->bd_dev));
    ...
    if(MAJOR(bdev->bd_dev) != LOOP_MAJOR) {
        ...
        printk(KERN_INFO "%s: device name is %s\n", MODNAME, d_name);
    } else {
        printk(KERN_INFO "%s: device is a loop device\n", MODNAME);
...
int snapshot_remove_device(const char *dev_name) {
    ...
    printk(KERN_INFO "%s: snapshot_remove_device called for device %s\n", MODNAME, dev_name);
    ...
    printk(KERN_INFO "%s: device %s not found in mounted devices list\n", MODNAME, dev_name);
    ...
    list_for_each_entry_safe(p, tmp, &nonmounted_devices_list, list) {
        if (strncmp(p->dev_name, dev_name, SNAPSHOT_DEV_NAME_LEN) == 0) {
            ...
            printk(KERN_INFO "%s: device %s removed from non-mounted devices list\n", MODNAME, dev_name);
...
int snapshot_handle_mount(struct dentry *dentry, const char *timestamp) {
    ...
    printk(KERN_INFO "%s: device %s found in non-mounted devices list\n", MODNAME, d_name);
    ...
    if (d_name_path[0] == '/') {
        ...
        memmove(d_name_path, d_name_path+1, len);  /* sposta tutto di un byte a sinistra, incluso il ‘\0’ */
    }
...
/* PARTE SLEEPABLE  */
    ...
    printk(KERN_INFO "%s: creating snapshot path subdirectory at %s\n", MODNAME, dir_path);
    ...
    printk(KERN_INFO "%s: snapshot path subdirectory created at %s\n", MODNAME, dir_path);
...
exists:
    ...
    m_dev->dev = bdev->bd_dev; // Salva major e minor del device
...
static void snapshot_worker(struct work_struct *work) {
    ...
    rcu_read_unlock();
    if(!found) {
        printk(KERN_ERR "%s: device has no snapshot activated\n", MODNAME);
        goto out_free;
    }
...
found_dev:
    ...
    if (block_nr >= m_dev->bitmap_size) {
        rcu_read_unlock();
        printk(KERN_ERR "%s: block number %llu out of range for device %llu\n", MODNAME, (unsigned long long)block_nr, (unsigned long long)dev);
        return -EINVAL;
    }    

    if (test_and_set_bit(block_nr, m_dev->block_bitmap)) {
        rcu_read_unlock();
        printk(KERN_INFO "%s: block %llu on device %llu already marked as modified\n", MODNAME, (unsigned long long)block_nr, (unsigned long long)dev);
        return 0;
    }
    printk(KERN_INFO "%s: block %llu on device marked as modified\n", MODNAME, (unsigned long long)block_nr);
    ...
    list_for_each_entry_safe(blk, tmp, &m_dev->block_list, list) {
        if (blk->block_nr == block_nr && blk->bdev == bdev && blk->size == bh->b_size) {
            list_del(&blk->list);
            printk(KERN_INFO "%s: block %llu on device removed from block list\n", MODNAME, (unsigned long long)block_nr);
            found_blk = blk;
            break;
        }
    }

    spin_unlock(&m_dev->block_list_lock); // Rilascio lock sulla lista dei blocchi

    if (!found_blk) {
        printk(KERN_ERR "%s: block %llu on device not found in block list\n", MODNAME, (unsigned long long)block_nr);
        return -ENOENT;
    }
    ...
    work->data = found_blk->data; // usa i dati del blocco
...
found_dev:
    ...
    if (size != m_dev->block_size || size > PAGE_SIZE) {
        printk(KERN_ERR "%s: block size %zu does not match device block size %zu\n", MODNAME, size, m_dev->block_size);
        rcu_read_unlock();
        return -EINVAL;
    }

    if (test_bit(block_nr, m_dev->block_bitmap)) {
        printk(KERN_INFO "%s: block %llu on device already marked as modified\n", MODNAME, (unsigned long long)block_nr);
        rcu_read_unlock();
        return 0;
    }
    ...
    printk(KERN_INFO "%s: block %llu on device added to block list\n", MODNAME, (unsigned long long)block_nr);
    return 0;
}
// #endif // USE_BREAD
